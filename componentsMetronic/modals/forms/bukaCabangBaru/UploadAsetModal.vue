...
export default defineComponent({
  ...
  methods: {
    ...
    async refreshData() {
      if (
        ...
        hideModal(this.UploadAsetModal);
        return false;
      }
      ...
      try {
        await ApiService.get(
          `asset/cabang?cabang_bisnis_unit_id=${this.filter.bunit}&drtgl=${tglAwal}&sdtgl=${tglAkhir}`
        ).then((data) => {
  ...
  setup() {
    const UploadAsetModal = ref<HTMLDListElement | null>(null);
    ...
    return { UploadAsetModal, formRef, Copy, filter };
  },
});
</script>
