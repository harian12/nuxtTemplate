      ...
      <div class="modal-content">
        ...
        <el-form @submit.prevent="searchItems()">
          <div class="modal-body py-10 px-lg-17">
            <div class="row mb-2">
              <div class="col-3">
                <label for="" style="font-size: 16px">Group payment</label>
              </div>
              <div class="col-9">
                <Multiselect
                  v-model="filter.group_payment"
                  :searchable="true"
                  :close-on-select="false"
                  :groups="true"
                  mode="tags"
                  :options="option.group_payment"
                  placeholder="Select"
                ></Multiselect>
              </div>
            </div>
...
export default defineComponent({
  ...
  props: ["groupPayment", "handleFilter"],
  data() {
    return {
      filter: {
        group_payment: [],
  ...
  updated() {
    this.option.group_payment = [
      {
        label: "Select All",
        options: this.$props.groupPayment,
      },
    ] as never;
  },
  methods: {
